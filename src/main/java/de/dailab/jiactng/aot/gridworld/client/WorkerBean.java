...
public class WorkerBean extends AbstractAgentBean {
    ...
    private void sortOrders() {
        // TODO
        // idee: schaue immer zwei orders an, wenn eine in der aktuellen reihenfolge nicht abgeschlossen werden kann, tausche die orders und gehe weiter, erstmal ohne nochmal zu kontrollieren
        // vergleich anhand deadline und gesamtweg
        // oder einfach earliest deadline first?
        Order[] orders = (Order[]) priorityQueue.toArray();

        Integer dist = this.position.distance(orders[0].position)+1;
        for (int i = 1; i < orders.length; i++) {

            if(orders[i].deadline < time+dist+orders[i].position.distance(orders[i-1].position)){
                Order tmp = orders[i];
                orders[i] = orders[i-1];
                orders[i-1] = tmp;
            }
            dist+=orders[i].position.distance(orders[i-1].position)+1;
        }
    }
